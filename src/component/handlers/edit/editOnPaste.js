...
function editOnPaste(editor: DraftEditor, e: SyntheticClipboardEvent<>): void {
    ...
    if (html) {
      ...
      if (htmlFragment) {
        ...
        if (contentBlocks && contentBlocks.length) {
          const currentBlockData = RichTextEditorUtil.getCurrentBlockData(
            editorState,
          );

          const headBlock = contentBlocks[0].merge({data: currentBlockData});
  ...
  if (textBlocks.length) {
    ...
    const headBlock = textFragment[0].merge({data: currentBlockData});

    const textMap = BlockMapBuilder.createFromArray([
      headBlock,
